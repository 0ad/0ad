...
private:
	 ...
	 */
	static bool LoadLibrary(ScriptInterface::RealmPrivate* pRealmPrivate, const VfsPath& name);

	/**
	 * Finalize map generation and pass results from the script to the engine.
	 */
	static void ExportMap(ScriptInterface::RealmPrivate* pRealmPrivate, JS::HandleValue data);

	/**
	 * Load an image file and return it as a height array.
	 */
	static JS::Value LoadHeightmap(ScriptInterface::RealmPrivate* pRealmPrivate, const VfsPath& src);

	/**
	 * Load an Atlas terrain file (PMP) returning textures and heightmap.
	 */
	static JS::Value LoadMapTerrain(ScriptInterface::RealmPrivate* pRealmPrivate, const VfsPath& filename);

	/**
	 * Sets the map generation progress, which is one of multiple stages determining the loading screen progress.
	 */
	static void SetProgress(ScriptInterface::RealmPrivate* pRealmPrivate, int progress);

	/**
	 * Microseconds since the epoch.
	 */
	static double GetMicroseconds(ScriptInterface::RealmPrivate* pRealmPrivate);

	/**
	 * Return the template data of the given template name.
	 */
	static CParamNode GetTemplate(ScriptInterface::RealmPrivate* pRealmPrivate, const std::string& templateName);

	/**
	 * Check whether the given template exists.
	 */
	static bool TemplateExists(ScriptInterface::RealmPrivate* pRealmPrivate, const std::string& templateName);

	/**
	 * Returns all template names of simulation entity templates.
	 */
	static std::vector<std::string> FindTemplates(ScriptInterface::RealmPrivate* pRealmPrivate, const std::string& path, bool includeSubdirectories);

	/**
	 * Returns all template names of actors.
	 */
	static std::vector<std::string> FindActorTemplates(ScriptInterface::RealmPrivate* pRealmPrivate, const std::string& path, bool includeSubdirectories);
