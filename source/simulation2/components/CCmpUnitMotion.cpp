 ...
 */
static const u8 MAX_FAILED_PATH_COMPUTATIONS = 12;
...
void CCmpUnitMotion::PathResult(u32 ticket, const WaypointPath& path)
{
	...
	m_ExpectedPathTicket.m_Ticket = 0; // Keep ticket metadata
	...
	if (m_ExpectedPathTicket.m_Type == Ticket::LONG_PATH)
	{
		m_LongPath = path;
	...
	if ((pos - targetPos).CompareLength(LONG_PATH_MIN_DIST) <= 0)
	{
		PathGoal goal;
