 ...
 * USA.
 *
 * $Id: fontenum.cpp 598 2007-03-07 20:13:28Z fbraem $
 */
// fontenum.cpp
...
{
	JSObject *enumObj = GetObject();

	jsval fval;
    JSContext *cx = GetContext();
	if ( GetFunctionProperty(cx, GetObject(), "onFacename", &fval) == JS_TRUE )
	{
		jsval rval;
...
bool FontEnumerator::OnFontencoding(const wxString& faceName, const wxString &encoding)
{
	...
	jsval fval;
	if ( GetFunctionProperty(cx, GetObject(), "onFontEncoding", &fval) == JS_TRUE )
	{
		jsval rval;
		jsval argv[] = {  ToJS(cx, faceName)
						, ToJS(cx, encoding) };
 ...
 * </ctor>
 */
FontEnumerator* FontEnumerator::Construct(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, bool constructing)
{
