...
bool wxjs::gui::InitClass(JSContext *cx, JSObject *global)
{
	...
	obj = CheckListBoxChecked::JSInit(cx, global);
	wxASSERT_MSG(obj != NULL, wxT("wxCheckListBoxChecked prototype creation failed"));
	if (! obj )
		return false;
...
bool wxjs::gui::InitObject(JSContext *cx, JSObject *obj)
{
	...
	if ( ! wxApp::IsMainLoopRunning() )
	{
        JSObject *app = JS_ConstructObject(cx, App::GetClass(), App::GetClassPrototype(), NULL);
	    JS_DefineProperty(cx, obj, "wxTheApp", OBJECT_TO_JSVAL(app), NULL, NULL, 0);
	}
...
{
	// Only exit wxTheApp, if there is no running mainloop, becuase in this
	// case there exists still an instance of wxApp, which is the case when embedding
	// wxJS into a wxWidgets application
    if ( wxTheApp )
    {
	    ...
	    if ( ! wxApp::IsMainLoopRunning() && t_strClassName.IsSameAs(_("wxJSApp")) )
	    {
		    wxTheApp->OnExit();
		    // Cleanup 
		    #ifdef __WXMSW__
