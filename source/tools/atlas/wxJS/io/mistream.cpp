 ...
 * USA.
 *
 * $Id: mistream.cpp 598 2007-03-07 20:13:28Z fbraem $
 */
#include <wx/wxprec.h>
#ifndef WX_PRECOMP
    #include <wx/wx.h>
...
MemoryInputStream::MemoryInputStream(  JSContext *cx 
											 , JSObject *obj
											 , char *data
...
Stream* MemoryInputStream::Construct(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, bool constructing)
{
	if ( JSVAL_IS_OBJECT(argv[0]) )
	{
		...
		if ( buffer != NULL )
		{
			...
			return new Stream(new MemoryInputStream(cx, obj, dataPtr, (size_t) buffer->GetDataLen()));
		}		
	}
    ...
    if ( length > 0 )
    {
    	...
    	return new Stream(new MemoryInputStream(cx, obj, buffer, (size_t) length));
    }
    return NULL;
}
