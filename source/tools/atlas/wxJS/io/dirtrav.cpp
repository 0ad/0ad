 ...
 * USA.
 *
 * $Id: dirtrav.cpp 598 2007-03-07 20:13:28Z fbraem $
 */
...
DirTraverser::DirTraverser(JSContext *cx, JSObject *obj)
        : wxDirTraverser()
        , Object(obj, cx)
{
}
...
wxDirTraverseResult DirTraverser::OnFile(const wxString& filename)
{
	...
	jsval fval;
	if ( GetFunctionProperty(cx, GetObject(), "onFile", &fval) == JS_TRUE )
	{
		jsval rval;
...
wxDirTraverseResult DirTraverser::OnDir(const wxString& filename)
{
	JSContext *cx = GetContext();
	jsval fval;
	if ( GetFunctionProperty(cx, GetObject(), "onDir", &fval) == JS_TRUE )
	{
		jsval rval;
		jsval argv[] = { ToJS(cx, filename) };
		if ( JS_CallFunctionValue(cx, GetObject(), fval, 1, argv, &rval) == JS_TRUE )
...
{
    return new DirTraverser(cx, obj);
}
