/**
 * For historic reference, see
 * http://www.jebelbarkal.org/images/maps/siteplan.jpg
 */
...
function* GenerateMap(mapSettings)
{
	...
	const oTreasuresCity =
		[oFoodTreasureBin, oFoodTreasureCrate, oFoodTreasureJars].concat(oTreasuresHill);
	...
	const placeNapataWall = mapSize < 192 || getDifficulty() < 2 ? false : getDifficulty() < 3 ?
		"napata_palisade" : "napata_wall";
	...
	/**
	 * The buildings are set as uncapturable, otherwise the player would gain
	 * the buildings via root territory and can delete them without effort.
	...
	].filter(building => getDifficulty() >=
		getDifficulties().find(difficulty => difficulty.Name == building.difficulty).Difficulty);
	...
	for (const area of irrigationCanalAreas)
	{
		if (!area.getPoints().length ||
			area.getPoints().some(point => !avoidClasses(
				clPlayer, scaleByMapSize(8, 13),
				clIrrigationCanal, scaleByMapSize(15, 25)).allows(point)))
			continue;
	...
	for (let i = 0; i < irrigationCanalLocations.length; ++i)
	{
		...
		const next = i == irrigationCanalLocations.length - 1 ? mapBounds.right :
		...
		// The passages should be at different locations, so that enemies
		// can't attack each other easily
	...
	for (let y = 0; y < gridPointsY; ++y)
		for (let x = 1; x < gridPointsX; ++x)
		{
			...
			areasCityPaths.push(
				createArea(
					new PathPlacer(cityGridPosition[y][x - 1], cityGridPosition[y][x], width, 0,
						8, 0, 0, Infinity),
					new TileClassPainter(clPath)));
		}
	...
	for (let y = 1; y < gridPointsY; ++y)
		...
		{
			const width =
				Math.abs(x - gridPointXCenter) == 0 ? pathWidthCenter :
					Math.abs(x - gridPointXCenter) == 1 ? pathWidthSecondary : pathWidth;

			areasCityPaths.push(
				createArea(
					new PathPlacer(cityGridPosition[y - 1][x], cityGridPosition[y][x], width, 0,
						8, 0, 0, Infinity),
					new TileClassPainter(clPath)));
		}
	yield 70;
	...
	for (let i = 0; i < layoutKushTemples.length; ++i)
	{
		...
		templePosition[i] = Vector2D.add(cityGridPosition[0][x],
			layoutKushTemples[i].pathOffset.rotate(-Math.PI / 2 - cityGridAngle[0][x]));
		entitiesTemples[i] = g_Map.placeEntityPassable(layoutKushTemples[i].template, 0,
			templePosition[i], cityGridAngle[0][x]);
	}
	...
	const centralPathLength = centralPathStart.distanceTo(
	...
	for (let i = 0; i < 2; ++i)
		...
		{
			const start = new Vector2D(0, pathWidthCenter * 3/4 * (i - 0.5))
				.rotate(centralPathAngle)
				.add(centralPathStart);
			const position = new Vector2D(centralPathLength, 0)
			...
			g_Map.placeEntityPassable(pickRandom(aStatues), 0, position,
	...
	for (let i = 0; i < 2; ++i)
		...
		{
			const start = new Vector2D(0, pathWidthCenter * 1/2 * (i - 0.5))
				.rotate(-centralPathAngle)
				.add(centralPathStart);
			const position = new Vector2D(centralPathLength, 0)
			...
			g_Map.placeEntityPassable(pickRandom(oKushChampions), 0, position,
				centralPathAngle - Math.PI * (i - 0.5));
			clPathStatues.add(position.round());
		}
	...
	{
		g_Map.placeEntityAnywhere(aStatueKush, 0, cityGridPosition[gridPointYCenter][x],
	...
	const ritualAngle =
		(cityGridAngle[0][gridPointXCenter] + cityGridAngle[0][gridPointXCenter - 1]) / 2 + Math.PI / 2;
	...
	createObjectGroupsByAreas(
		new SimpleGroup([new SimpleObject(oTriggerPointCityPath, 1, 1, 0, 0)], true,
		...
		for (let x = 1; x < gridPointsX; ++x)
			createArea(
				...
				[
					...
					new CityPainter(layoutKushCity,
	...
	if (placeNapataWall)
	{
		...
		const wallGridPositionFront =
			distributePointsOnCircularSegment(gridPointsX, wallGridMaxAngleBack, wallGridStartAngle,
				wallGridRadiusFront, gridCenter)[0];
		const wallGridPositionBack =
			distributePointsOnCircularSegment(gridPointsX, wallGridMaxAngleBack, wallGridStartAngle,
				wallGridRadiusBack, gridCenter)[0];
		const wallGridPosition =
			[
	...
	const areaCityPaths =
		new Area(areasCityPaths.reduce((points, area) => points.concat(area.getPoints()), []));
	const areaRoads = [];
	for (const roadStart of roadStartLocations)
	{
		...
		for (let tries = 0; tries < 30; ++tries)
		{
			const area = createArea(
				new PathPlacer(
					Vector2D.add(closestPoint,
						new Vector2D(0, 3/4 * mapSize).rotate(closestPoint.angleTo(roadStart))),
					roadStart,
					scaleByMapSize(5, 3),
					0.1,
					5,
	...
	for (let i = 0; i < (isNomad() ? scaleByMapSize(6, 16) : scaleByMapSize(0, 8)); ++i)
	{
		...
		createObjectGroupsByAreas(
			...
			0,
			avoidCollisionsMines.concat([
				avoidClasses(clDesert, 5, clMetal, 15, clRock, 15, mineObjectsBiome.tileClass, 20)
			]),
			1,
			80,
			[areaFertileLand]);
	...
	createObjectGroups(
		new SimpleGroup([new SimpleObject(oTriggerPointAttackerPatrol, 1, 1, 0, 0)], true,
			clTriggerPointMap),
		0,
		[avoidClasses(
			clCity, 8,
	...
	createObjectGroupsByAreas(
		new SimpleGroup([new RandomObject([oKushCitizenArcher, oKushChampionArcher],
			scaleByMapSize(4, 10), scaleByMapSize(6, 20), 1, 4)], true, clSoldier),
		0,
	...
	createObjectGroupsByAreas(
		new SimpleGroup([new RandomObject([oKushCitizenArcher, oKushChampionArcher], 1, 1, 1, 3)], true,
			clSoldier),
		0,
		new StaticConstraint([
