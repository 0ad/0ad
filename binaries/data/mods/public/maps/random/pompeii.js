...
function* GenerateMap(mapSettings)
{
	...
	const areaVesuv = createArea(
		...
		[
			new LayeredPainter([g_Terrains.lavaOuter, g_Terrains.lavaInner, g_Terrains.lavaCenter],
	...
	createObjectGroups(
		new SimpleGroup(
			g_Decoratives.shipwrecks.map(shipwreck => new SimpleObject(shipwreck, 0, 1, 1, 20)),
			true,
			g_TileClasses.decorative),
		0,
		[
	...
	createObjectGroups(
		new SimpleGroup(g_Decoratives.statues.map(ruin => new SimpleObject(ruin, 0, 1, 1, 20)), true,
			g_TileClasses.decorative),
		0,
		avoidClasses(
			g_TileClasses.water, 2,
