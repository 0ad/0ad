...
function* GenerateMap()
{
	...
	{
		createArea(
			new ClumpPlacer(size * diskArea(playerIslandRadius), 0.95, 0.6, Infinity,
				islandPos[islandID]),
			[
	...
	function createIslandAtRadialLocation(playerID, islandID, playerIDOffset, distFromCenter,
		islandRadius)
	{
		const angle = startAngle + (playerID * 2 + playerIDOffset) * Math.PI / numPlayers;
		islandPos[islandID] = Vector2D.add(mapCenter,
			new Vector2D(fractionToTiles(distFromCenter), 0).rotate(-angle)).round();
		...
		islandBetweenPlayersDist, islandBetweenPlayersRadius])
	{
		...
		for (let playerID = 0; playerID < numPlayers; ++playerID)
		{
			...
			createIslandAtRadialLocation(playerID, islandID_betweenPlayers, 1,
				islandBetweenPlayersDist, islandBetweenPlayersRadius);
			...
			createIslandAtRadialLocation(playerID, islandID_betweenPlayerAndCenter, 0,
			...
			createIslandAtRadialLocation(playerID, islandID_tertiary, 1, tertiaryIslandDist,
				tertiaryIslandRadius);
		}
	}

	/**
	 * Creates one island in front of every player and connects it with the
	 * neighbors.
	 */
	function createSnowflakeSearockWithoutCenter()
	{
		...
		for (let playerID = 0; playerID < numPlayers; ++playerID)
		{
			...
			createIslandAtRadialLocation(playerID, islandID_inFrontOfPlayer, 0,
				islandBetweenPlayerAndCenterDist, islandBetweenPlayerAndCenterRadius);
		}
	}
