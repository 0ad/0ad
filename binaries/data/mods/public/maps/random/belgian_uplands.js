...
const terrainTypes = [
	...
	{
		"upperHeightLimit": heightSeaGroundAdjusted + 1/6 * (heightRange.max - heightSeaGroundAdjusted),
		"terrain": [
			Array(48).fill([
				"temp_plants_bog",
				"temp_plants_bog_aut",
	...
	{
		"upperHeightLimit": heightSeaGroundAdjusted + 5/6 * (heightRange.max - heightSeaGroundAdjusted),
		"terrain": [
			...
			for (let k = 0; k < resourceCount; ++k)
				g_Map.placeEntityPassable(
					...
					Vector2D.sum([
						...
						new Vector2D(-0.75 * resourceSpacing * Math.floor(resourceCount / 2), 0)
