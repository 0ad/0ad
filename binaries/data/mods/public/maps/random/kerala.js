...
function* GenerateMap()
{
	...
	placePlayerBases({
		"PlayerPlacement": [
			sortAllPlayers(),
			playerPlacementLine(0, new Vector2D(mapCenter.x, playerPosition), fractionToTiles(0.2))
	...
	{
		const position =
			new Vector2D(fractionToTiles(randFloat(0.28, 0.34)), fractionToTiles(randFloat(0.1, 0.9)))
				.rotateAround(startAngle - Math.PI / 2, mapCenter).round();
		createArea(
