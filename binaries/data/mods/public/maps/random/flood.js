...
function* GenerateMap()
{
	...
	for (let i = 0; i < numPlayers; ++i)
		createArea(
			new ClumpPlacer(diskArea(1.4 * defaultPlayerBaseRadius()), 0.8, 0.1, Infinity,
				playerPosition[i]),
			[
	...
	createStragglerTrees(
		types,
		[
			avoidClasses(clBaseResource, 2, clMetal, 6, clRock, 6, clMountain, 2, clPlayer, 25),
			stayClasses(clHill, 6)
		],
		clForest,
	...
	createFood(
		...
		[
			avoidClasses(clForest, 0, clPlayer, 20, clMountain, 1, clFood, 4, clRock, 6, clMetal, 6),
			stayClasses(clHill, 6)
		],
		clFood);
	...
	createFood(
		...
		[3 * numPlayers],
		[
			avoidClasses(clForest, 0, clPlayer, 15, clMountain, 1, clFood, 4, clRock, 6, clMetal, 6),
			stayClasses(clHill, 6)
		],
		clFood);
