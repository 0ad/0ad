...

{
	...
	const heightmapLand =
		convertHeightmap1Dto2D(Engine.LoadHeightmapImage("maps/random/lower_nubia_heightmap.png"));
	const heightmapLandThreshold =
		convertHeightmap1Dto2D(Engine.LoadHeightmapImage("maps/random/lower_nubia_land_threshold.png"));
	...
	for (let x = 0; x < heightmapLand.length; ++x)
	{
		...
		for (let y = 0; y < heightmapLand.length; ++y)
			// Reduce ahistorical Lake Nasser and lakes in the valleys
			//	west of the Nile.
			// The heightmap does not correlate with water distribution in
			//	this arid climate at all.
