...
function* GenerateMap()
{
	...
	// For large maps there are memory errors with too many trees. A density
	//	of 256x192/mapArea works with 0 players.
	// Around each player there is an area without trees so with more players
	//	the max density can increase a bit.
	...
	{
		playerAngle[i] = (playerAngleStart + i * playerAngleAddAvrg + randFloat(0, playerAngleMaxOff)) %
			(2 * Math.PI);
		playerPosition[i] =
		...
		for (let j = pathBlending ? 0 : i + 1; j < numPlayers + 1; ++j)
		{
			...
			createArea(
				...
				[
					...
					new SmoothElevationPainter(ELEVATION_SET, heightPath, 2,
	...
	for (let i = 0; i < numPlayers; ++i)
		for (let rIndex = 0; rIndex < resourcePerPlayer.length; ++rIndex)
		{
			...
			// they are supposed to be in between players on the same
			// radius
			const angle = playerAngle[i] + angleDist * (rIndex + 1) / (resourcePerPlayer.length + 1);
			const position = Vector2D.add(mapCenter, new Vector2D(resourceRadius, 0).rotate(-angle))
				.round();
	...
	for (let x = 0; x < mapSize; x++)
		for (let z = 0; z < mapSize; z++)
		{
			...
			// General height map
			const hVarMiddleHill = fractionToTiles(1 / 64) *
