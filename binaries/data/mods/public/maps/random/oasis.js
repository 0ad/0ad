...

{
	...
	globalThis.g_Map = new RandomMap(heightLand, tSand);
	...
	for (let i = 0; i < numPlayers; ++i)
	{
		...
		do
		{
			...
			forestPosition =
				Vector2D.add(playerPosition[i], new Vector2D(forestDist, 0).rotate(-forestAngle));
		} while (
			!createArea(
		...
		do
		{
			...
			waterPosition =
				Vector2D.add(forestPosition, new Vector2D(6, 0).rotate(-waterAngle)).round();
			flowerPosition =
				Vector2D.add(forestPosition, new Vector2D(3, 0).rotate(-waterAngle)).round();
			reedsPosition =
				Vector2D.add(forestPosition, new Vector2D(5, 0).rotate(-waterAngle)).round();
		} while (
			!createArea(
		...
		createObjectGroup(new SimpleGroup([new SimpleObject(aFlower1, 1, 5, 0, 3)], true, undefined,
			flowerPosition), 0);
	...
	placePlayerBases({
		...
		"Mines": {
			...
			"maxAngle": Math.PI / 2,
			"groupElements": shuffleArray([aBushA, aBushB, ePalmShort, ePalmTall])
				.map(t => new SimpleObject(t, 1, 1, 3, 4))
		}
	...
	createArea(
		new ClumpPlacer(diskArea(forestDistance + shoreDistance + waterRadius), 0.8, 0.2, Infinity,
			mapCenter),
		[
			new LayeredPainter([pOasisForestLight, tWater], [forestDistance]),
			new SmoothElevationPainter(ELEVATION_SET, heightSeaGround,
	...
	if (mapSize > 150 && randBool())
	{
		...
		const points = distributePointsOnCircle(2, randomAngle(),
			waterRadius + shoreDistance + forestDistance + pathWidth, mapCenter)[0];
		createArea(
	...
	{
		const animalPos =
			Vector2D.add(mapCenter, new Vector2D(forestDistance + shoreDistance + waterRadius, 0)
				.rotate(randomAngle()));
		...
		for (let sandz = 0; sandz < mapSize; sandz += 4)
		{
			...
			if (height > heightSand)
			{
				if (randBool((height - heightSand) / 1.4))
					createObjectGroup(new SimpleGroup([new SimpleObject(aSand, 0, 1, 0, 2)], true,
			...
			else if (height > heightFloraMin && height < heightFloraMax)
			{
				if (randBool(0.4))
					createObjectGroup(
						new SimpleGroup(
							[new SimpleObject(aWaterFlower, 1, 4, 1, 2)], true, undefined,
							position),
						0);
				...
				if (clPassage.countMembersInRadius(position, 2))
				{
					if (randBool(0.4))
						createObjectGroup(
							new SimpleGroup(
								[new SimpleObject(aWaterFlower, 1, 4, 1, 2)], true,
								undefined, position),
							0);
					else if (randBool(0.7) && height < heightFloraReedsMax)
						createObjectGroup(
	...
	placePlayersNomad(clPlayer,
		avoidClasses(clOasis, 4, clForest, 1, clMetal, 4, clRock, 4, clHill, 4, clFood, 2));
