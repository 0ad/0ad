...
function* GenerateMap(mapSettings)
{
	...
	{
		g_Terrains.mainTerrain = new Array(3).fill("desert_dirt_rough_2")
	...
	globalThis.g_Map = new RandomMap(0, g_Terrains.mainTerrain);
	...
	for (let i = 0; i < scaleByMapSize(4, 10); ++i)
		createObjectGroupsByAreas(
			randBool() ?
				new SimpleGroup([new SimpleObject(g_Gaia.metalLarge, 1, 1, 0, 4)], true,
					g_TileClasses.metal) :
				new SimpleGroup([new SimpleObject(g_Gaia.stoneLarge, 1, 1, 0, 4)], true,
					g_TileClasses.rock),
			0,
			[
	...
	// Prevent the water from disappearing on the tiny mapsize while
	// maximizing land on greater sizes
