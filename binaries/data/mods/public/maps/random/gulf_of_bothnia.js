...
function* GenerateMap(mapSettings)
{
	...
	placePlayerBases({
		"PlayerPlacement": [sortAllPlayers(), ...playerPlacementCustomAngle(
			...
			mapCenter,
			i => startAngle + 1/3 * Math.PI * (1 + 2 * (numPlayers == 1 ? 1 :
				2 * i / (numPlayers - 1))))],
	...
	{
		const position =
			Vector2D.add(mapCenter, new Vector2D(gulfLake.x, 0).rotate(-startAngle)).round();
	...
	if (randBool())
		...
		createMountains(tCliff, avoidClasses(clPlayer, 20, clHill, 15, clLake, 0), clHill,
			scaleByMapSize(1, 4) * numPlayers, Math.floor(scaleByMapSize(20, 40)));
	...
	if (!isLakeFrozen)
	{
		...
		// but since this placer would fail most of the time on the frozen
		// lake -> save some time by just skipping it
