...

{
	...
	for (let i = 0; i < numPlayers; ++i)
		for (let j = 1; j <= 2; ++j)
			createArea(
				new ClumpPlacer(diskArea(playerCanyonRadius / j), 0.65, 0.1, Infinity,
					playerPosition[i]),
				[
		...
		if (newarea !== null)
		{
			...
			for (const playerID of [p1, p2])
				if (playerPosition[playerID])
					createArea(
						new PathPlacer(position, playerPosition[playerID],
							scaleByMapSize(11, 17), 0.4, scaleByMapSize(3, 12), 0.1, 0.1),
						[
							new TerrainPainter(tMainTerrain),
							new SmoothElevationPainter(ELEVATION_SET, heightLand, 3),
							new TileClassPainter(clLand)
	...
	createHills([tCliff, tCliff, tHill], [avoidClasses(clPlayer, 2, clHill, 8, clHill2, 8),
		stayClasses(clLand, 5)], clHill, scaleByMapSize(10, 40));

	// create hills outside the canyon
	createHills([tCliff, tCliff, tMainTerrain], avoidClasses(clLand, 1, clHill, 1), clHill,
		scaleByMapSize(20, 150), undefined, undefined, undefined, undefined, 40);
	...
	createMines(
		...
		[
			avoidClasses(clForest, 1, clPlayer, 3, clMetal, 10, clRock, 5, clHill, 1, clHill2, 1),
			stayClasses(clLand, 2)
		],
		clMetal
	);
