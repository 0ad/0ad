...

{
	...
	const teamPosition =
		distributePointsOnCircle(numTeams, startAngle, fractionToTiles(0.3), mapCenter)[0];
	...
	for (let i = 0; i < teams.length; ++i)
	{
		...
		const [playerPosition, playerAngle] = distributePointsOnCircle(teams[i].length,
			startAngle + 2 * Math.PI / teams[i].length, teamRadius, teamPosition[i]);
		...
		for (let p = 0; p < teams[i].length; ++p)
			...
			{
				const position = Vector2D.add(playerPosition[p],
					new Vector2D(g_InitialMineDistance, 0).rotate(-playerAngle[p] - mine.angle));
				createObjectGroup(
					new SimpleGroup([new SimpleObject(mine.template, 1, 1, 0, 4)], true,
						clBaseResource, position),
					0,
		...
		for (let p = 0; p < teams[i].length; ++p)
		{
			...
			for (let x = 0; x < tries; ++x)
			{
				...
				const treePosition =
					Vector2D.add(playerPosition[p], new Vector2D(16, 0).rotate(-tAngle)).round();
		...
		for (let p = 0; p < teams[i].length; ++p)
			placePlayerBaseBerries({
				...
				"baseResourceConstraint":
					new AndConstraint([avoidClasses(clPlayer, 4), stayClasses(clLand, 5)])
			});
	...
	createForests(
		...
		[
			avoidClasses(clPlayer, 10, clForest, 20, clBaseResource, 5, clRock, 6, clMetal, 6),
	...
	createFood(
		...
		[3 * numPlayers],
		[
			avoidClasses(clForest, 0, clPlayer, 15, clHill, 1, clFood, 4, clRock, 6, clMetal, 6),
			stayClasses(clLand, 2)
		],
		clFood);
	...
	if (currentBiome() == "generic/sahara")
	{
		...
		createObjectGroupsDeprecated(
			group, 0,
			[
				avoidClasses(clBaseResource, 0, clHill, 0, clRock, 0, clMetal, 0, clFood, 0),
