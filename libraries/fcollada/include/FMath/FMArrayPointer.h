...
namespace fm
{
	...
	public:
		...
		pvector(size_t size) : Parent(size) {}
		...
		T*& front() { return (T*&) Parent::front(); }
		const T*& front() const { return (const T*&) Parent::front(); } /**< See above. */

		/** Retrieves the last element of the pointer array.
			@return The last element in the pointer array. */
		T*& back() { return (T*&) Parent::back(); }
		const T*& back() const { return (const T*&) Parent::back(); } /**< See above. */
		...
		inline const_iterator find(const T* item) const { Parent::const_iterator f = Parent::find(item); return begin() + (f - Parent::begin()); }
		...
		/** Inserts a list of pointers in the pointer array.
			...
			@param startIterator The iterator for the first pointer to insert.
			@param endIterator The iterator for the last pointer.
		...
		/** Removes the value at the given position within the pointer array.
			@param it The position for the pointer to remove. */
